...
func Run(ctx context.Context, svc *todo.Service, addr string, creds credential.Source, stdout io.Writer) error {
	server := &http.Server{
		Addr: addr,
		...
		Handler:           NewMux(svc, RequireProtocolVersion, Authenticate(creds)),
